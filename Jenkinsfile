pipeline{
    ...
    environment {
        IMAGE_NAME = "manoj_s/microservices-platform/order-service:${GIT_COMMIT}"
    }

    stages{
        stage('Git Checkout'){
            steps{
                git url: 'https://github.com/KubeNexa/order-service.git' , branch: 'main'
            }
        }
            ...
            steps{
                withCredentials([usernamePassword(
                    ...
                    sh '''
                        ...
                        sed -i "s|image: .*orderservice.*|image: ${IMAGE_NAME}|g" deployment.yaml
